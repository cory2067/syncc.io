...
#            self.lineseg((i)/(N-0.5),-envelope[i]/smax,(i+1)/(N-0.5),-envelope[i+1]/smax)
    ...
    def update(self, K):
        ...
        response = self.makeSF(self.K).simulator().get_response([1]+29*[0])

        #find the dominant pole and its associated coefficient
        dompole_ix = max(range(len(poles)), key=lambda x: abs(poles[x]))
        dompole = poles[dompole_ix]
        dompole_mag = abs(dompole)

        title = 'K = {0:f}     abs(dominant pole) = {1:f}'.format(self.K,dompole_mag)
        self.viewResponse(title,poles,response,None)
